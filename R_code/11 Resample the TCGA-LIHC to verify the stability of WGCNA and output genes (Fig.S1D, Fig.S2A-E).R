...
enableWGCNAThreads()
# Repeatedly sample 50 times from the TCGA-LIHC cohort to certain the stability of WGCNA
...
tmf1 = system.time ( {
  mods1 = sampledBlockwiseModules(
    ...
    minModuleSize = 30,
    mergeCutHeight = 0.2,
    reassignThreshold = 6,checkMissingData = FALSE,quickCor = 0,
    verbose = 3 ) } )
print(tmf1)

# Take the mods1 result and create a big plot
nRuns=50
nGenes = ncol(expr_5000)
nSamples = nrow(expr_5000)
labels = matrix(0, nGenes, nRuns)
labels[, 1] = mods1[[1]]$mods$colors

# Match labels from each resampling to the labels obtained for the full data set
pind = initProgInd()
for (r in 1:50)
{
  labels[, r] = matchLabels(mods1[[r]]$mods$colors, labels[, 1])
  pind = updateProgInd((r-1)/nRuns, pind)
}

# Plot a big plot of the main dendrogram and 50 color bars underneath.
pdf(file = "Resampling_50_287.pdf", wi=20, h=15)
plotDendroAndColors(mods1[[1]]$mods$dendrograms[[1]],
...
# Select 1/2 of the samples from the TCGA-LIHC cohort as a training set
# Load samples and gene expression data 
sample_50<-read.table(file = "SAMPLE_50.txt", sep = "\t",header = T)
sample_237<-read.table(file = "SAMPLE_237.txt", sep = "\t",header = T)
# Merge the samples and gene expression data after random sampling
sample_50_25<-sample_50 %>% sample_n(25, replace = FALSE,set.seed(1234))
sample_237_118<-sample_237 %>% sample_n(118, replace = FALSE,set.seed(1234))
sample_143<-rbind(sample_50_25,sample_237_118)
expr<-read.table(file = "HiSeqV2.txt", sep = "\t",header = T,row.names = 1)
expr_50_25<-expr[,sample_50_25$Sample]
expr_237_118<-expr[,sample_237_118$Sample]
expr_143<-cbind(expr_50_25,expr_237_118)
trait <- read.table("trait.txt",sep = "\t",header=T,row.names = 1)
trait <- trait[sample_143$Sample,]
updown<-read.table(file = "up&down_5000.txt", sep = "\t",header = T)
expr_5000<-expr_143[updown$Gene,]
...
module = "tan"
column = match("MEtan", colnames(MEs_col))
moduleGenes = moduleColors==module
tan_module<-as.data.frame(dimnames(data.frame(expr_5000))[[2]][moduleGenes]) 
names(tan_module)="genename"
hub<- abs(geneModuleMembership$MM2)>0.80 & abs(geneTraitSignificance)>0.2
hubgene_tan <- dimnames(data.frame(expr_5000))[[2]][hub]
MM<-abs(geneModuleMembership[moduleGenes,column])
GS<-abs(geneTraitSignificance[moduleGenes, 1])
tan<-as.data.frame(cbind(MM,GS))
rownames(tan)=tan_module$genename
match<- tan_module$genename %in% hubgene_tan
tan$group<-match
write.table(tan, file="tan.txt", sep="\t", quote=F, row.names = T)

module = "turquoise"
column = match("MEturquoise", colnames(MEs_col))
moduleGenes = moduleColors==module
turquoise_module<-as.data.frame(dimnames(data.frame(expr_5000))[[2]][moduleGenes]) 
names(turquoise_module)="genename"
hub<- abs(geneModuleMembership$MM2)>0.80 & abs(geneTraitSignificance)>0.2
hubgene_turquoise <- dimnames(data.frame(expr_5000))[[2]][hub]
MM<-abs(geneModuleMembership[moduleGenes,column])
GS<-abs(geneTraitSignificance[moduleGenes, 1])
turquoise<-as.data.frame(cbind(MM,GS))
rownames(turquoise)=turquoise_module$genename
match<- turquoise_module$genename %in% hubgene_turquoise
turquoise$group<-match
write.table(turquoise, file="turquoise.txt", sep="\t", quote=F, row.names = T)
